...
| <a id="opt-acme-certificatesDuration" href="#opt-acme-certificatesDuration" title="#opt-acme-certificatesDuration">`acme.certificatesDuration`</a> | The certificates' duration in hours, used to determine renewal times if ARI is broken or disabled. | 2160 | No |
| <a id="opt-acme-disableARI" href="#opt-acme-disableARI" title="#opt-acme-disableARI">`acme.disableARI`</a> | Disable ACME Renewal Information (ARI); fall back to time-based renewal scheduling.                                                                                                                                                                                     | false                                          | No       |
...
By default, Traefik uses the ACME Renewal Information (ARI) extension to let the certificate authority guide Traefik when to renew a certificate. Before renewing, Traefik will query the CA's `renewal-info` endpoint and renews within the window suggested. This will let CAs trigger an early renewal for mass-revocation events.

If the CA does not support ARI, Traefik falls back to a time-based schedule derived from the `certificatesDuration` setting. You can also set `disableARI` to `true` to always use the time-based schedule and never query the ARI endpoint.

### Time-Based Renewal

If ARI is not available (or is disabled or the endpoint is misbehaving), Traefik will renew on a set timetable. For example, 90-day certificates will be renewed 30-days prior to expiration.
