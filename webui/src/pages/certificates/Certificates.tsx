...
import {computeDaysLeft, computeFractionRemaining} from 'hooks/use-certificates'
...
export const CertificateRenderRow: RenderRowType = (row: unknown) => {
  ...
  const fractionRemaining = computeFractionRemaining(cert.notBefore, cert.notAfter)
  ...
  return (
    <ClickableRow key={cert.name} to={`/certificates/${cert.name}`}>
      ...
      <AriaTd>
        <CertExpiryBadge fractionRemaining={fractionRemaining} daysLeft={daysLeft} size="small" />
      </AriaTd>
    </ClickableRow>
  )
}
